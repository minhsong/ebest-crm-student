...
/**
 * Quên mật khẩu — gọi API thống nhất, quản lý loading / lỗi.
 */
export function useForgotPassword() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const submit = useCallback(async (loginId: string) => {
    setError(null);
    setLoading(true);
    try {
      const result = await postForgotPassword(loginId);
...
/**
 * Đặt lại mật khẩu — token từ query email (CRM chấp nhận purpose Lead hoặc Customer).
 */
