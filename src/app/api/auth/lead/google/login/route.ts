...
/** Lead Google login — set cookie lead JWT; auto-verify email phía CRM. */
export async function POST(request: Request) {
  ...
  const url = buildCrmStudentUrl(apiBase, STUDENT_API.authLeadGoogleLogin);
  ...
  const token =
    typeof payload?.accessToken === 'string'
      ? (payload.accessToken as string).trim()
      : '';
  if (token) {
    setPortalSessionCookie('lead', token);
  }

  const out = { ...payload, actor: 'lead' as const };
